...
function decodeObjectRefs(ptr, length, memory) {
	...
	for (let i = 0; i < count; i++) result[i] = memory.getUint32(basePtr + 4 * i, true);
	return result;
}
	...
	get wasmImports() {
		...
		return {
			...
			swjs_call_function_with_this_no_catch: (obj_ref, func_ref, argv, argc, payload1_ptr, payload2_ptr) => {
				...
				const func = memory.getObject(func_ref);
				const args = decodeArray(argv, argc, this.getDataView(), memory);
				return writeAndReturnKindBits(func.apply(obj, args), payload1_ptr, payload2_ptr, false, this.getDataView(), this.memory);
			},
	...
	return {
		addImports: (importObject, importsContext) => {
			...
			bjs["swift_js_init_memory"] = function(sourceId, bytesPtr) {
				...
				new Uint8Array(memory.buffer, bytesPtr >>> 0).set(source);
			};
			bjs["swift_js_make_js_string"] = function(ptr, len) {
				return swift.memory.retain(decodeString(ptr, len));
			};
			bjs["swift_js_init_memory_with_result"] = function(ptr, len) {
				new Uint8Array(memory.buffer, ptr >>> 0, len >>> 0).set(tmpRetBytes);
				tmpRetBytes = void 0;
			};
		...
		createExports: (instance) => {
			swift.memory.heap;
			structHelpers.M14BrowserInteropT23JSKeyframeEffectOptions = __bjs_createStructHelpers_M14BrowserInteropT23JSKeyframeEffectOptions();
			structHelpers.M14BrowserInteropT17JSAnimationTiming = __bjs_createStructHelpers_M14BrowserInteropT17JSAnimationTiming();
			return {
